...
Matrix *matrix_create(int row, int col, int buffer_s) {
    ...
    matrix->m = row;
    matrix->total_m = row + buffer_s;
    matrix->n = col;
    double *data;
    int total = (matrix->total_m) * col;

    data = (double *) malloc(total * sizeof(double));
    matrix->entries = (double **) malloc(matrix->total_m * sizeof(double *));

    for (int i = 0; i < matrix->total_m; i++) {
...
ParMatrix *create_output_matrix_size(ParMatrix *X, int new_n, int buffer_s) {
    ...
    output->gm = X->gm;
    output->gn = X->gn;
    output->inPart = X->inPart;
    output->l2gMap = X->l2gMap;
    output->store = X->store;
    output->mat = matrix_create(X->mat->m, new_n, buffer_s);
    return output;
}

ParMatrix *create_output_matrix(ParMatrix *X) {
    return create_output_matrix_size(X, X->mat->n, 0);
}
