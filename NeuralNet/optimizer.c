...
void adam_step(Matrix *gradients, Matrix *gradients_bias, gcnLayer *layer, double lr, double beta1, double beta2,
    ...
    double beta1_corr = 1 - pow(beta1, t + 1);
    double beta2_corr = 1 - pow(beta2, t + 1);
    for (int i = 0; i < gradients->m; i++) {
        for (int j = 0; j < gradients->n; j++) {
            ...
            layer->v_weights->entries[i][j] = beta2 * layer->v_weights->entries[i][j] +
                                              (1 - beta2) * grad_sqr;
            double mHat = layer->m_weights->entries[i][j] / beta1_corr;
            double vHat = layer->v_weights->entries[i][j] / beta2_corr;
    ...
    for (int j = 0; j < layer->size_output; j++) {
        ...
        double mHat = layer->m_bias[j] / beta1_corr;
        double vHat = layer->v_bias[j] / beta2_corr;
